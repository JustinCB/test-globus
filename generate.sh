...
cat functions.sh endpoint_test.ini | awk 'NF != 0{sub(/^[\t ]*/,"");gsub(/#[A-TV-fhj-ln-rstvwz ][A-z0-9 ][A-z0-9@\/\-\&,#\'\''\\\(\)"\$ ]+$/,"");gsub(/[\t ]*$/,"");if($0!=""){print}}' >endpoint_test.sh
chmod +x endpoint_test.sh
cat functions.sh test_globus.ini | awk 'NF != 0{sub(/^[\t ]*/,"");gsub(/#[A-TV-fhj-ln-rstvwz ][A-z0-9 ][A-z0-9@\/\-\&,#\'\''\\\(\)"\$ ]+$/,"");gsub(/[\t ]*$/,"");if($0!=""){print}}' >test_globus.sh
