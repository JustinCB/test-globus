...
cat functions.sh endpoint_test.ini | awk '$0 != ""{sub(/^[\t ]*/,"");gsub(/#[A-TV-fhj-ln-rtvwz ][A-z0-9@\/\-\&,#\'\'' ]+$/,"");print}' >endpoint_test.sh
chmod +x endpoint_test.sh
cat functions.sh test_globus.ini | awk '$0 != ""{sub(/^[\t ]*/,"");gsub(/#[A-TV-fhj-ln-rtvwz ][A-z0-9@\/\-\&,#\'\'' ]+$/,"");print}' >test_globus.sh
